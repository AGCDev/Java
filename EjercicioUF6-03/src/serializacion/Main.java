...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		try {
			do {
				...
				opcion = Integer.parseInt(datos.readLine());// Tenia opcion = sc.nextInt(); pero me devolvía un error
															// que no comprendo, asi que, lo he sustituido por este
															// ultimo...
	 ...
	 */
	// Crear archivo clientes.dat, lo mejor es crearlo previa y manualmente, pero lo
	// usaremos para otros métodos
	...
	// Método para abrir el archivo y cargue toda la información que contiene, la
	// información la lee el ordenador, NO confundir con lectura humana, por eso
	// input
	private static void abrirCargarArchivoClientes() throws IOException, ClassNotFoundException {

		try {
			// Si no existe el archivo "clientes.dat", llamará al método crearArchivo(), en
			// caso contrario, nos indiracará que ya existe
			if (!archivoClientes.exists()) {
				crearArchivo();
			} else {
				System.out.println("El archivo ya existe");
			...
			if (archivoClientes.canRead()) {
				// Se introduce un objeto al archivo, en este caso los clientes, las personas
				// también son objetos en java (POO)
				try (FileInputStream fis = new FileInputStream(archivoClientes);
						// Se introduce un objeto al archivo, en este caso los clientes, las personas
						// también son objetos en java (POO)
						ObjectInputStream ois = new ObjectInputStream(fis)) {
					// A esto se le llama hacer casting o castear, sirve para convertir tipos, es
					// parecido a parsear
	...
	private static void escribirArchivoClientes() throws IOException {
		...
		try {
			// Si no existe el archivo, crealo, en caso contrario, envia mensaje diciendo
			// que ya existe
			if (!archivoClientes.exists()) {
				crearArchivo();
			} else {
			...
			try ( // Nos muestra el archivo, por eso OutPut
					FileOutputStream fos = new FileOutputStream(archivoClientes); // Creamos un objeto para que nos lo
																					// muestre
